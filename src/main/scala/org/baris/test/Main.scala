...
object Main extends App {
  ...
  private val conf = Conf.live("world")
  private val dbRecoveryConf = conf >>> ZLayer.fromService { (c: Conf.Root) =>
    c.dbRecovery
  }
  ...
  val appEnv = zenv ++ conf ++ database ++ personQueries
  ...
  def myApp(): ZIO[AppEnv, DbException, List[Person]] = {
    ...
    ZIO.accessM[AppEnv] { env =>
      ...
      implicit val errorRecovery: ErrorStrategiesRef =
        env.get[Conf.Root].alternateDbRecovery
